(function ($) {
    ...
    $('.navbar-collapse.collapse').on('show.bs.collapse', function (e) {
        ...
        if (!($(e.target).closest('.navbar-nav').length || $(e.target).closest('.navbar-full-screen-menu-inner').length)) { setTimeout(function () { $('.navbar-collapse.collapse').collapse('hide'); }, 100); }
    ...
    $('.nav-item  .nav-link').on('click', function (e) {
        var submenuLenth = $(this).closest('.dropdown').find('.dropdown').length; if ($(window).width() < 992 && $(this).closest('.dropdown').length > 0) {
            var HasClass = $(this).closest('.nav-item').find('.dropdown-menu').hasClass('show'); if ($(this).closest('.nav-item').find('.dropdown-menu').hasClass('show')) {
                $(this).closest('.nav-item').find('.dropdown-menu').removeClass('show')
                $(this).closest('.nav-item').find('.dropdown-toggle').removeClass('show')
            }
            else {
                $('.navbar-nav').find('.dropdown-menu').removeClass('show'); $('.navbar-nav').find('.dropdown-toggle').removeClass('show'); $(this).closest('.nav-item').find('.dropdown-menu').addClass('show')
    ...
    $(window).on('load', function () {
        ...
        if ($('nav.header-reverse-back-scroll').length > 0) {
            var st = scrollPos; if (st > lastScroll) { st = st - 1; $('header').removeClass('sticky-appear'); } else { $('header').addClass('sticky-appear'); }
